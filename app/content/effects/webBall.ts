...
interface Props {
    ...
    az?: number
    source: Enemy
    activate?: (state: GameState, grenade: WebBall) => void
}

export class WebBall implements EffectInstance, Props {
    ...
    defaultActivate(state: GameState) {
        playAreaSound(state, this.area, 'barrierBurst');
        const webBlast = new Blast({
            x: this.x,
            y: this.y,
            radius: this.radius,
            damage: this.damage,
            ...
            tellDuration: 0,
            source: this.source,
            hitProperties: {isWebbed: true, cutsGround: false},
        });
        addEffectToArea(state, this.area, webBlast);
    }
...
// damage = 1, z = 8,
export function throwWebBallAtLocation(state: GameState, enemy: Enemy, {tx, ty}: {tx: number, ty: number},  props: ThrowGrenadeProps): void {
    ...
    addEffectToArea(state, enemy.area, webBall);
}
